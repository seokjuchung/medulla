...
namespace ana::tools
{
   ...
   * You need to tell the analisys_tree where the signal/selected neutrinos/selected cosmics live, supply an initial selection cut, and
                                                                                                 ...
                                                                                                 const ana::tools::cut_sequence& old_truth_cut)
  {
    ...
    TTree* bkgFromBkg = nuBackgroundTree->CopyTree(opt.cut.c_str());
    ...
    nuBackgroundTree = std::move(newNuBackgroundTree);
    std::unique_ptr<TTree> newCosmicTree = std::unique_ptr<TTree>(cosmicTree->CopyTree(opt.cut.c_str()));
    newCosmicTree->SetDirectory(memFile.get());
    cosmicTree = std::move(newCosmicTree);
    total_signal = signalTree->GetEntries();
                                                                                                     ...
                                                                                                     const ana::tools::cut_sequence& truth_cut) const
  {
    ...
    unsigned int selected_background  = signalTree        ->GetEntries(cut.with_addition(ana::tools::negate_cut(truth_cut)).c_str());
                 selected_background += nuBackgroundTree  ->GetEntries(cut.c_str());
