...
inline std::vector<std::pair<std::string, ana::tools::cut_sequence>> sel_cats =
{
  ...
  {"Topology Failure",           "true_category == 3"},
  {"Uncontained",                "true_category == 4"},
  {"Non-Fiducial",               "true_category == 5"},
  {"Cosmic",                     "true_category == 6"}
};
                 ...
                 const bool& optimizeCuts)
{
  ...
  ana::tools::analysis_tree my_analysis_tree(fileLocation+"/"+fileName, directoryName, "(true_category == 0)",
  ...
  my_analysis_tree.add_variable("reco_gOre_ke",                 20,     0,   2000,    "#gamma-candiate KE (MeV)");
  ...
  for (auto const& var : my_analysis_tree.variables())
  {
        ...
        var == "true_res_code"         )
    {
      std::function<std::string(double)> get_label = (var == "true_interaction_type") ? [](const int& code){ return (genie_inttypes.count(code) == 1) ? genie_inttypes.at(code) : ""; }
                                                   ...
                                                   : (var == "true_res_code")         ? [](const int& code){ return res_codes.at(code); }
      ...
      std::string title_str = (var == "true_interaction_type") ? "GENIE Interaction Type;;Events"
                            ...
                            : (var == "true_res_code")         ?              "Resonance;;Events"
                            :                                                          ";;Events";
...
int main(int argc, char* argv[])
{
  ...
  ret = try_call("Run gOre Analysis", run_analysis, fileName, "gOre", "gOre", "gOre", optimize_cuts);
  return ret;
}